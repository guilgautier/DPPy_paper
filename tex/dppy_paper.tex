...
\label{sec:introduction}
  ...
  We use Travis\footnote{\url{https://travis-ci.com/guilgautier/DPPy}} for continuous integration. %, both to run tests and certify that \DPPy\ can be used with Linux and MacOS.
...
\label{sec:determinantal_point_processes}
  ...
  \label{sub:sampling}
      ...
      \end{equation}
      Finally, the algorithm boils down to expressing the chain rule for \Eqref{eq:joint_distribution1}.
      To be precise, let us denote by $\Phi(x) \triangleq \lrp{\phi_{i_1}(x),\dots,\phi_{i_N}(x)}$ the so-called \emph{feature vector} associated with $x\in\bbX$. This yields $\tilde{K}(x,y) = \Phi(y)^{\dagger} \Phi(x)$. More importantly, we can now rewrite the joint distribution in \Eqref{eq:joint_distribution1} as
      \begin{equation}
      ...
      In applications where these costs are a bottleneck, users can rely on approximate sampling procedures. Research has focused on two main directions: kernel approximation and MCMC samplers\todo{Add a few references}.

      To conclude, some specific DPPs admit more efficient exact samplers that do not rely on \Eqref{eq:joint_distribution2}, e.g., uniform spanning trees \citep{PrWi98} or eigenvalues of random matrices. For instance, so-called $\beta$-ensembles are defined by the joint distribution
      \begin{equation*}
        \frac{1}{Z_{N,\beta}}
