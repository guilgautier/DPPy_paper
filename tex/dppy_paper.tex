...
\label{sec:determinantal_point_processes}
  ...
  \label{sub:definition}
    ...
    A point process $\calX$ on $(\bbX,\mu)$ parametrized by a kernel $K:\bbX\times \bbX\rightarrow \mathbb{C}$ is said to be \emph{determinantal}, denoted as $\calX\sim\DPP(K)$, if its $k$-correlation functions satisfy
	  ...
	  \end{equation}
		\noindent
    where $\bfK_S$ denotes the submatrix of $\bfK$ formed by the rows and columns indexed by $S$. If we further assume that the kernel matrix $\bfK$ is Hermitian, then existence and unicity of the DPP~\ref{eq:inclusion_proba_finite} is equivalent to the condition that the spectrum of $\bfK$ should lie in $[0,1]$, see \cite[Theorem 3]{Sos00}. We note that the latter assumptions are only sufficient. In particular, there exist DPPs with non symmetric kernels, see \citep{BoDiFu09}.
