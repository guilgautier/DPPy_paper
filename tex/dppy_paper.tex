...
\label{sec:introduction}
    ...
    \DPPy\ is hosted on GitHub,\!\footnoteref{fn:github}we use \setcounter{footnote}{5}Travis\!\footnote{\url{https://travis-ci.com/guilgautier/DPPy}}for continuous integration, Coveralls\!\footnote{\url{https://coveralls.io/github/guilgautier/DPPy}}for test coverage.
    ReadTheDocs \!\footnoteref{fn:docs}allows to provide an extensive documentation which collects the essential mathematical background and showcases the key properties of DPPs through \DPPy\ objects and associated methods.
    \DPPy\ thus also serves as a tutorial.

    Along the paper, words in magenta point to the documentation.
    In \Secref{sec:definitions}, we introduce DPPs. In \Secref{sec:sampling} we briefly review the challenging task of sampling
...
%     We introduce DPPs and the main sampling algorithm; see \citet{HKPV06} for details.
    ...
    \label{sec:definitions}
        ...
        Indeed, \Eqref{eq:inclusion_proba_finite} entails
        \begin{equation*}
    ...
    \label{sec:sampling}
        ...
        Then, one can apply the chain rule to sample $(X_1,\dots,X_N)$ with probability distribution given by \Eqref{eq:joint_distribution}, and then discard the order the points were drawn to get a valid sample $\lrcb{X_{1}, \dots, X_{N}} \sim \DPP(\tilde{K})$.
        \begin{equation}
        ...
        First, the LHS of \Eqref{eq:joint_distribution} is indeed an exchangeable probability distribution.
        % , the $N!$ factor captures all possible permutations.
