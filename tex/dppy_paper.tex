...
%     We introduce DPPs and the main sampling algorithm; see \citet{HKPV06} for details.
    ...
    \label{sec:sampling}
        ...
        Nonetheless specific DPPs admit more efficient exact samplers that do not rely on \Eqref{eq:joint_distribution}, \eg uniform spanning trees \citep{PrWi98} or eigenvalues of random matrices.
...
\label{sec:the_dppy_toolbox}
    ...
    \begin{figure*}[!hb]
        ...
        \centering
        \subfigure[]{
            \includegraphics[width=\threefig]{images/multivariate_jacobi_2D_1000points.pdf}
            \label{fig:diversity}
        }
%            \includegraphics[width=\threefig]{../img/bolt_volumes.pdf}
        % \qquad\qquad
        \subfigure[]{
            \includegraphics[width=\threefig]{images/LUE.pdf}
            \label{fig:volume}
        }
        \subfigure[]{
            \includegraphics[width=\threefig]{images/UST_kernel.pdf}
            \label{fig:volume}
        }
        \vspace{-2em}
        \caption{(a) A $N=1000$ sample of the 2D Jacobi ensemble (b) Histogram of the eigenvalue of the covariance matrix of $N=1000$ complex Gaussian vectors, rescaled to show the limiting Marčenko-Pastur distribution (c) Display of the $\bfK$ kernel associated to the uniform spanning tree distribution of a graph.}
        \label{fig:bolt}
    \end{figure*}
