...
\label{sec:introduction}
    ...
    The \DPPy\ project, hosted on GitHub,\!\footnoteref{fn:github} is already being used by the community (\citealp{BuRaWi19,Kam18,Pou19,DeCaVa19,GaBaVa19}).
    % \citep[?!][]{BaHa16}
...
%     We introduce DPPs and the main sampling algorithm; see \citet{HKPV06} for details.
    ...
    \label{sec:sampling}
        ...
        \end{equation*}
        In the finite case,
        % \href{https://dppy.readthedocs.io/en/latest/finite_dpps/exact_sampling.html}{\textcolor{magenta}{finite case}},
        ...
        \citep{Gil14,TrBaAm18}.
        Alternative exact samplers exist among which, the natural bottom-up  Cholesky-based procedure of \citet{Pou19} and the  \citet{DeCaVa19} which offers interesting cost reduction in certain regimes.
        ...
        For instance, a $\beta$-ensemble is a set of $N$ points $\{X_1,\dots,X_N\}\subset \bbR$ with probability distribution given by
        \begin{equation*}
        ...
        They are therefore examples of continuous DPPs that can be sampled exactly in $\calO(N^2)$, without rejection.
        ...
        It is worth mentioning that \DPPy\ implements a multivariate extension of the so-called \emph{Jacobi} ensemble that has become the spearhead of DPP-based Monte Carlo integration methods \citep{BaHa16,GaBaVa19,MaCoAm19}.
...
\label{sec:the_dppy_toolbox}
    ...
    \DPPy\ handles Python objects that fit the natural definition of the corresponding DPPs, for more details we refer to the documentation\footnoteref{fn:docs}and the corresponding \href{https://github.com/guilgautier/DPPy/tree/master/notebooks}{Jupyter notebooks}, which showcase \DPPy\ objects.
    For instance, a finite $\DPP(\bfK)$ can be defined as
    \lstinline!FiniteDPP(kernel_type="correlation", **{"K": K})!\!.
    Its two main methods, namely
    ...
    The Laguerre $\beta$-ensemble can be instantiated as
    ...
    \GG{Add some nice figures like MultivariateJacobi, Laguerre and kernel of UST ?}
