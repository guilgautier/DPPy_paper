...
%     We introduce DPPs and the main sampling algorithm; see \citet{HKPV06} for details.
    ...
    \label{sec:sampling}
        ...
        In the finite setting, this corresponds to an initial $\calO(M^3)$ eigendecomposition cost, followed by an average complexity of order $\calO(M\Expe{|\calX|}^2)$ \rb{Why not specify $\calO(M Tr(K)^2)$? See also below in the vost of Derezinski et al.}
        ...
        In the continuous case, sampling exactly each conditional that appears in the right-hand side of \Eqref{eq:joint_distribution} can be done using rejection sampling with a tailored proposal. \rb{Cite your NeurIPS paper?}
